...
def scanStubs():
    ...
    for dir in dir_list:
        ...
        program, fuzzer, index = dir.split("_")
        ...
        if fuzzer == "afl++":
            queue_dir = os.path.join(output_dir, dir, "default", "queue")
        else:
            queue_dir = os.path.join(output_dir, dir, "queue")

        if fuzzer == "carpetfuzz":
            argvs_path = os.path.join(repo_path, "configures", "carpetfuzz_dataset", "argvs", package, "ranked_stubs_%s.txt" % (program))
